...
client = OpenAI(
    api_key="sk-dsfkjdslkfjKey", 
    base_url="http://221.10.121.80:20002/v1"
    );



examples_data = {    # 示例数据
    '新闻报道': '今日，股市经历了一场震荡，受到宏观经济数据和全球贸易紧张局势的影响。投资者密切关注美联储可能的政策调整，以适应市场的不确定性。',
    '财务报告': '本公司年度财务报告显示，去年公司实现了稳步增长的盈利，同时资产负债表呈现强劲的状况。经济环境的稳定和管理层的有效战略执行对公司的健康',
...
for q in questions:
    response = client.chat.completions.create(
        # model="qwen3-max",
        model="gemma-4-31B_Q4_0",
        messages=messages + [{"role": "user", "content": f"按照示例，回答这段文本的分类类别：{q}"}]
    )

    
    print(f"{q}--->>> {response.choices[0].message.content}")